...
class Venda():
	...
	def add(self,Ingresso_Id,Usuario_Id,Quantidade,Preco,DataEmitida,Vendedor_Id):
		try:
			sql = "INSERT INTO %s (Ingresso_Id,Usuario_Id,Quantidade,Preco,DataEmitida,Vendedor_Id) VALUES (?,?,?,?,?,?)" % self.table
			self.cursor.execute(sql,[Ingresso_Id,Usuario_Id,Quantidade,Preco,DataEmitida,Vendedor_Id])
			self.db.commit()
			self.db.close()
			return True
		except Exception as e:
	...
	def pickupticket(self,Usuario_Id):
		try:
			sql = "SELECT evento.Titulo,evento.Endereco,evento.Bairro,evento.Numero,evento.Telefone,cidade.Nome,estado.Sigla,categoria.Nome,ingresso.Tipo,venda.Quantidade,venda.Preco,venda.DataEmitida FROM venda JOIN ingresso ON Ingresso.Id = venda.Ingresso_Id JOIN evento ON evento.Id = ingresso.Evento_Id JOIN cidade ON cidade.Id = evento.Cidade_Id JOIN estado ON estado.Id = cidade.Estado_Id JOIN categoria ON categoria.Id = evento.Categoria_Id WHERE venda.Usuario_Id = ?".format(venda=self.table,ingresso=self.ITable,evento=self.ETable,categoria=self.Ctable,cidade=self.CiTable,estado=self.EsTable)
			self.cursor.execute(sql,[Usuario_Id])
			return self.cursor.fetchall()
		except Exception as e:
			print e		
